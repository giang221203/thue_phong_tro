...
const isAgent = async (req, res, next) => {
  // check có phải khách hàng hay ko
...
const isAdmin = async (req, res, next) => {
  // check có phải admin hay ko
  const { roleCode } = req.user; // sau khi đã verify và lấy được token ,lấy được (req.user) thì chạy xuống check quyền
  if (roleCode !== "ROL1") {
    // nếu role khác rol1 tức không phải admin thì bắn lỗi
...
const isOwner = async (req, res, next) => {
  // check có phải chủ tài sản ,người muốn bán hay ko
